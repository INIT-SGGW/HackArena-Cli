...
pub async fn submit(
    ...
    let wrapper_kind = map_wrapper_kind(&project.edition, &manifest.wrapper.language)?;
...
fn map_wrapper_kind(edition: &str, language: &str) -> Result<WrapperKindMapped, HackArenaError> {
    let normalized = language.trim().to_ascii_lowercase();
    match normalized.as_str() {
        "python" => Ok(WrapperKindMapped::Python),
        _ => Err(HackArenaError::msg(format!(
            "Wrapper language `{language}` is not supported for submit. Only `python` is supported for official submission in edition `{edition}`."
        ))),
    }
}
...
mod tests {
    ...
    #[test]
    fn map_wrapper_kind_supports_python() {
        let kind = map_wrapper_kind("3", "python").expect("python should be supported");
        assert_eq!(kind as i32, 1);
    }

    #[test]
    fn map_wrapper_kind_rejects_non_python_languages() {
        let err = map_wrapper_kind("3", "typescript").expect_err("typescript should fail");
        assert!(err.to_string().contains("Only `python` is supported"));
    }
}
